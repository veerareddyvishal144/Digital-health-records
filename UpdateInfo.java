...
public class UpdateInfo extends JFrame {
  ...
  //declaring swing components
	   private JLabel l_name,l_pass,l_ht,l_wt;
	   private    JTextField t_name,t_ht,t_wt;
	 private   JPasswordField t_pass;     //A special JTextField but hides input text
	private JFrame ind;
	   private    JButton button;
	   private   JPanel []c;
private JFrame swing;
...
public   UpdateInfo(ResultSet ps,JFrame id,JFrame swing)
  {
...
setLayout(new GridLayout(5,1));
  ...
  class handler implements ActionListener
  {
      ...
      public void actionPerformed(ActionEvent ae)
      {
      	...
      	try
      	{
      	if(t_name.getText().length()==0||t_pass.getText().length()==0||t_ht.getText().length()==0||t_wt.getText().length()==0){
          ...
          //checks if the button clicked
              ...
              String query = "update pacc set name=?,password=?,height=?,weight=? where username=? ";
                  ...
                  preparedStmt.setString(5,rs.getString(1));
